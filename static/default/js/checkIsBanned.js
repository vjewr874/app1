...
function isThisAccountBannedFunc() {
    $.get('/source/index/ajax_profile.php?ac=accountIsBan',function (data) {
        if(data){
...
function showBanedAlert() {
    alert('因发布的APP不符合审核规则，或您发布的APP已违规，现已停用该账号发布功能', function () {
        // 是否在上传页，在上传页跳转到列表
        if(isUploadPage == 1) {
    ...
    // +      '</div>'
    // +     '<div style="padding: 25px 25px;color:#f00;font-size: 14px">因发布的APP不符合审核规则，或您发布的APP已违规，<br />已停用账号发布功能！</div>'
    // +   '</div>'
    // + '</div>'
    //
